...
void mcast_print_timeout_list(const struct multi_instance *mi, struct gc_arena *gc)
{
    ...
    //msg (D_MCAST_LOW, "MCAST: current timeouts for multi-instance. (now = %s)", tv_string_abs(&((struct timeval){now,0}), gc));

    for (current_list = mi->mcast_timeouts; current_list; current_list = current_list->next)
        msg(D_MCAST_LOW, "MCAST:   (%s, %s) -> %s", mroute_addr_print(current_list->rcpt, gc), print_in_addr_t(current_list->group, IA_EMPTY_IF_UNDEF, gc), tv_string_abs(&current_list->timeout, gc));
}

/** functions to manipulate mcast-receiver-lists */
struct mcast_recipient_list * mcast__create_recv_list_item(struct multi_instance * mi, const struct mroute_addr * rcpt)
{
...
void mcast__update_time_for_recipient(struct multi_instance * mi, const uint32_t group, struct mroute_addr * rcpt)
{
    ...
    *current_list = new_list_item;
    ...
    msg(D_MCAST_LOW, "MCAST: update time (%s, %s) -> %s",
            mroute_addr_print(new_list_item->rcpt, &gc),
            print_in_addr_t(new_list_item->group, IA_EMPTY_IF_UNDEF, &gc),
            tv_string_abs(&new_list_item->timeout, &gc));
    gc_free(&gc);
...
const struct openvpn_iphdr* mcast_extract_ipv4(const struct buffer *buf, int tunnel_type)
{
    const struct openvpn_iphdr *ip;
    const struct openvpn_ipv6hdr *ipv6;
    struct buffer nbuf = *buf;

    switch (tunnel_type) {
        case DEV_TYPE_TUN:
            /* Nothing special buffer 'b' is already at the right position */
            break;
        case DEV_TYPE_TAP:
            if (BLEN(&nbuf) < (int) sizeof (struct openvpn_ethhdr)) {
                return NULL;
            }
            if (!buf_advance(&nbuf, sizeof (struct openvpn_ethhdr))) {
                return NULL;
            }
            break;
        default:
            /* Wrong tunnel_type */
            msg(D_MULTI_LOW, "MCAST: Wrong tunnel type");
            return NULL;
    }
    ...
    if (BLEN(&nbuf) < 1) {
        /* Not have length */
        msg(D_MULTI_LOW, "MCAST: No length");
        return NULL;
    }
    switch (OPENVPN_IPH_GET_VER(*BPTR(&nbuf))) {
        case 4: /* IPv4 */
            if (BLEN(buf) < (int) sizeof (struct openvpn_iphdr)) {
                return NULL;
            }
            ip = (const struct openvpn_iphdr *) BPTR(buf);
            return ip;
#if 0
        case 6: /* IPv6 */
            ...
            ipv6 = (const struct openvpn_ipv6hdr *) BPTR(buf);
            if (ipv6->nexthdr != OPENVPN_IPPROTO_IGMP) {
                /* No IGMP */
                return NULL;
            }
            if (!buf_advance(&nbuf, sizeof (struct openvpn_ipv6hdr))) {
                return NULL;
            }
            return (const struct openvpn_igmpv3hdr *) nbuf.data;
#endif
        default:
            //msg(D_MULTI_LOW, "MCAST: Unknown IP version %02X", *BPTR(&nbuf));
            return NULL;
    }
    return NULL;
}

const struct openvpn_igmpv3hdr* mcast_extract_igmp(const struct buffer *buf, int tunnel_type)
{
    const struct openvpn_iphdr *ip;
    const struct openvpn_ipv6hdr *ipv6;
    struct buffer nbuf = *buf;
        ...
        int tunnel_type)
{
    ...
    if (ip = mcast_extract_ipv4(buf, tunnel_type)) {
